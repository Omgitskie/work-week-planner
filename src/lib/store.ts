import { AppData, Employee, AbsenceRecord } from './types';

const STORAGE_KEY = 'holiday-tracker-data';

const DEFAULT_STORES = ['London', 'Manchester', 'Birmingham', 'Leeds', 'Bristol'];

const DEFAULT_EMPLOYEES: Employee[] = [
  { id: '1', name: 'Alice Johnson', store: 'London' },
  { id: '2', name: 'Bob Smith', store: 'London' },
  { id: '3', name: 'Carol Williams', store: 'Manchester' },
  { id: '4', name: 'David Brown', store: 'Manchester' },
  { id: '5', name: 'Emma Davis', store: 'Birmingham' },
  { id: '6', name: 'Frank Wilson', store: 'Leeds' },
  { id: '7', name: 'Grace Taylor', store: 'Bristol' },
  { id: '8', name: 'Henry Moore', store: 'London' },
];
...
export function loadData(): AppData {
  try {
    ...
    if (raw) {
      ...
      // Ensure stores field exists for backwards compat
      if (!parsed.stores) parsed.stores = DEFAULT_STORES;
      if (!parsed.year) parsed.year = 2026;
      return parsed;
    }
  } catch {
    // ignore
